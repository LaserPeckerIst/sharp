...
import com.angcyo.svg.ImageTracerAndroid;
        ...
        b2.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View view) {
                try {
                    ...
                    log(" Error tracing panda.png " + e.toString());
                    e.printStackTrace();
                }
            }
        });
    ...
    public class Optionselector {
            ...
            for (String s : opt.keySet()) {
                ...
                sbs.get(thiss).setOnSeekBarChangeListener(new OnSeekBarChangeListener() {
                    ...
                    public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
                        float thisval = 0;
                        if (progress < 50) {
                            thisval = opt.get(thiss)[1] + ((progress) * ((opt.get(thiss)[0] - opt.get(thiss)[1]) / 50f));
                        } else {
                            thisval = opt.get(thiss)[0] + ((progress - 50) * ((opt.get(thiss)[2] - opt.get(thiss)[0]) / 50f));
                        }
        ...
        public HashMap<String, Float> getvals() {
            ...
            for (String s : opt.keySet()) {
                float v = opt.get(s)[0];
                try {
                    v = Float.parseFloat(ets.get(s).getText().toString());
                } catch (Exception e) {
                    log("Error parsing float. " + s + " [" + ets.get(s).getText().toString() + "]");
                }
                o.put(s, v);
            }
            return o;
